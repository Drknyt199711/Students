...
# Bot API and user IDs from the prompt
...
# --- COURSE DATA ---
# This is a placeholder. In a real-world scenario, you might load this from a database.
COURSES = {
...
# These are integer states for the enrollment conversation
(
    ENROLLMENT_START,
    GET_FULLNAME,
    GET_PHONE,
...
def update_github_repo(username, repo_name, file_path, new_content, token):
    ...
    sha = None
    existing_content = ""
    # Check if the file exists to get its SHA and current content
    ...
    if response.status_code == 200:
        ...
        # File doesn't exist, so we prepare to create it with a header
    ...
    payload = {
        ...
        "sha": sha,  # Only required for updating an existing file
    }
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Check if the user is already enrolled (a placeholder for now)
    ...
    # If not enrolled, prompt for enrollment
    ...
    # We return the state ENROLLMENT_START to the ConversationHandler
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data.clear()  # Clear any stored user data
    return ConversationHandler.END
...
async def enroll_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Move to the next state to receive the full name
    return GET_FULLNAME
...
async def get_fullname(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Move to the next state to receive the phone number
    return GET_PHONE
...
async def get_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not re.match(r'^(251|0)?[97]\d{8}$', phone_number):
        ...
        return GET_PHONE # Stay in the same state
    ...
    # Move to the next state to receive the course selection
...
async def choose_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if course_info:
        ...
        # Move to the next state to receive the payment receipt
...
async def upload_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Store the photo file ID in user_data
    ...
    # Send all details to the admin for verification
    verification_text = (
    ...
    # End the enrollment conversation for the user
    return ConversationHandler.END
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "verify":
        # Get the course name from user_data and retrieve the group ID
        ...
        # Create a one-time, 24-hour invitation link
        try:
            invite_link = await context.bot.create_chat_invite_link(
                chat_id=COURSE_GROUP_ID,
                member_limit=1,  # Only one user can join with this link
                expire_date=int(time.time()) + 86400  # Link expires in 24 hours (86400 seconds)
            )
            
            # Send the link to the verified user
            await context.bot.send_message(
                chat_id=user_id,
            ...
            # Inform the admin that the link has been sent
            await context.bot.send_message(
                chat_id=ADMIN_GROUP_ID,
                text=f"✅ A one-time invitation link has been successfully sent to user {user_id} for the *{course_name}* course."
            )
            
            # Update the original verification message to show it's been handled
...
def main():
    ...
    # Conversation handler for the enrollment process
